    ...
    public function add() {
       ...
       /* function valid_email($str) {
        ...
        // validation success
			...
			if($count == 1 ){
                 echo "<script> alert('vous etes deja inscrit dans la filiere : $titre ');</script>" ;
                 $this->load->view('inscription_form');
                
            }else{
                $id_con =$condidat->getlastcondidat();
                $num = $id_con+1;
               $num_insc = $num.'/'.$num_insc.'/'.date('Y');  
               $condidat->numero_inscription = $num_insc;
               $condidat->save();
			   
			   $diplome->id_condidat = $id_con;
			   $inscrit_formation->id_condidat = $id_con;
			  // die();
